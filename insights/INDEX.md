...
| [062](062-flatland-no-costless-restraint-lever-open-population-dies-within-one-predator-generation.md) | Flatland front rung 3b PRECONDITION (Part A; DESIGN gate ruled the evolved-restraint experiment DEAD ON ARRIVAL -- 061 collapses in ~1 predator generation so selection cannot act): does any regime PERSIST for many predator GENERATIONS? NO -- a SQUEEZE: the only restraint available is decline-to-hunt = eat nothing, so greedy (theta=1, non-starving) collapses by OVER-CONSUMPTION and any restraint (theta>1) accelerates extinction by STARVATION; no COSTLESS restraint lever exists and that coupling IS the finding. 0.00 persistence (coexist AND >=20 pred generations) across theta x density (0/90), a predator-energetics sweep (0/54), AND the 061 strong-stabiliser corner satiate=12/percept=2 (0/6); max ~1.4 generations << GMIN 20. So no eco-evolutionary timescale -> rung 3b (evolved restraint, now rung 063) BLOCKED in this region. SCOPE: the searched region + decline-to-hunt family + these energetics (world size/perception/prey-repro/plant-energy FIXED), NOT open ALife in general; a substrate change is a HYPOTHESIS not a demonstrated necessity | confirmed negative (no eco-evo timescale; restraint-starvation squeeze) + method (design REDESIGN dead-on-arrival->precondition; claim SIGN-WITH-CHANGES strong-corner+squeeze+scoping) | 2026-07-24 |
